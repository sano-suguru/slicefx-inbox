...
{
    public record Response(string Id, string Url, string Title, string? Description, string Status, DateTimeOffset SavedAt, string Source, string[]? Tags);
    ...
    public static async Task<WasiResponse> Handle(string id, IKeyValueStore kv, CancellationToken ct)
    {
        ...
        return SliceResult.Ok(
            new Response(item.Id, item.Url, item.Title, item.Description, item.Status, item.SavedAt, item.Source, item.Tags),
