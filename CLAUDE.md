...
src/Inbox.Server/
  ...
  Infrastructure/ISecrets.cs             auth token abstraction + SafeEquals
  Infrastructure/SpinKeyValueStore.cs    wasi:keyvalue WIT-bound IKeyValueStore impl
  Infrastructure/SpinVariables.cs        fermyon:spin/variables WIT-bound ISpinVariables (raw WIT)
...
Reference impl context: `SpinVariables.cs` uses raw WIT (`VariablesInterop.Get`) rather than
the `ISpinVariables` abstraction added in preview.5. This is intentional — the file serves
as a concrete reference for the binding shape. After bumping to preview.5, swapping to
`ISpinVariables` is optional; raw WIT is valid and shows the full binding detail.
...
in NativeAOT-LLVM WASI builds. `Infrastructure/ISecrets.cs::TokenAuth.SafeEquals` uses a manual
